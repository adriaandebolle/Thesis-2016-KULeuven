...
ySteps = int((2*Rc)//(2*Rs))
zSteps = int(ySteps//math.sqrt(3))
...
for i in range (0,lengthSteps) :
	...
	for l in range (-zSteps,zSteps) :
		...
		if centreToCentre < Rc-Rs :
			...
			#geompy.addToStudy(p, 'p'+str(i)+str(k)+str(l))
...
geompy.DifferenceList( Rest, inletFaces + outletFaces + inletWalls + outletWalls + internal + poroInletFaces + poroOutletFaces + poroWalls + pinternal1 + pinternal2 + pinternal3 )
